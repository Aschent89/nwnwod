////////////////////////////////////////////////////////////////////////////////
//
//  Olander's Binding Stones/Bleeding/Death
//  obd_portal
//  By Don Anderson
//  dandersonru@msn.com
//
//  Original Idea by Dom Queron (Thank You)
//
//  This Portal will return the Player to either
//  the last saved Binding Stone or 1 of 3 Locations (Good, Evil, Neutral)
//
////////////////////////////////////////////////////////////////////////////////

#include "obd_include"

void main()
{
    object oPC = GetLastUsedBy();
    object oMod = GetModule();
    string sDB = OBD_GetDBString();

    float fDelay = 2.5;

    effect eFX1 = EffectVisualEffect(VFX_IMP_BREACH);
    effect eFX2 = EffectVisualEffect(VFX_IMP_LIGHTNING_M);
    effect eFX3 = EffectVisualEffect(VFX_IMP_RAISE_DEAD);
    effect eFX4 = EffectVisualEffect(VFX_FNF_SUMMON_MONSTER_2);
    effect eFX5 = EffectVisualEffect(VFX_IMP_RESTORATION_GREATER); // For Healing at Respawn Area
    effect eFX6 = EffectVisualEffect(VFX_IMP_HEALING_G); //For after Raise Dead effect at Bind Stone or Home

    //Change Appearance Back to Normal
    int nRace = GetRacialType(oPC);
    int nDefApp = StringToInt(Get2DAString("racialtypes","Appearance",nRace));
    int nDefPheno;
    int nPheno = GetPhenoType(oPC);
    if(nPheno == PHENOTYPE_BIG || nPheno == 8 || nPheno == 5) nDefPheno = PHENOTYPE_BIG;
    else {nDefPheno = PHENOTYPE_NORMAL;}
    SetCreatureAppearanceType(oPC,nDefApp);
    SetPhenoType(nDefPheno,oPC);
    SetCreatureTailType(CREATURE_TAIL_TYPE_NONE,oPC);
    SetFootstepType(FOOTSTEP_TYPE_NORMAL,oPC);
    NBDE_SetCampaignInt(sDB, "ORS_PC_APPEARANCE", nDefApp, oPC);

    //New Players are in The Void...When Using the Portal they go to Start Location.
    object oVoid = GetNearestObjectByTag("WP_VOID");
    if(GetIsObjectValid(oVoid))
    {
      DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFX1, oPC));
      DelayCommand(fDelay + 1.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFX2, oPC));
      DelayCommand(fDelay + 2.0, AssignCommand(oPC, ActionJumpToLocation(GetStartingLocation())));
      DelayCommand(fDelay + 3.0, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectKnockdown(), oPC, 3.0));
      DelayCommand(fDelay + 4.0, ApplyEffectToObject (DURATION_TYPE_INSTANT, eFX3, oPC));
      DelayCommand(fDelay + 5.0, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectDazed(), oPC, 5.0));
      DelayCommand(fDelay + 6.0, ApplyEffectToObject (DURATION_TYPE_INSTANT, eFX4, oPC));
      return;
    }

    //Check For a Death Deed
    object oTargetPerm = GetWaypointByTag("WP_PERMDEATH");
    object oDD         = GetItemPossessedBy(oPC, "DeathDeed");
    if(oDD != OBJECT_INVALID)
    {
      AssignCommand(oPC, ClearAllActions(TRUE));
      SendMessageToPC(oPC,"Your time in the living has passed. You can no longer leave here.");
      return;
    }

    //Portal Cooldown is Active
    int nCooldown = GetLocalInt(oPC,"OBD_PORTAL_DEL_COUNT");
    if(nCooldown > 0)
    {
      AssignCommand(oPC, ClearAllActions(TRUE));
      SendMessageToPC(oPC,"Portal Cooldown Timer is Active...Please wait to use Portal!");
      return;
    }

    int nNeutral = GetLocalInt(oMod,"NEUTRAL_ALLOW");
    if(nNeutral < 1 && GetAlignmentGoodEvil(oPC) == ALIGNMENT_NEUTRAL)
    {
      SendMessageToPC(oPC,"You MUST be Good or Evil Aligned!! Neutral is NOT Accepted! A DM can help you or make a new character.");
      return;
    }

    string sDestTagBS = NBDE_GetCampaignString(sDB,"BS_LASTBINDPOINT",oPC);
    string sDestTagDF =  "";
    object oSpawnPointBS = GetObjectByTag(sDestTagBS);

    // Checks to see if the Bindpoint is valid
    if(GetIsObjectValid(oSpawnPointBS))
    {
      DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFX1, oPC));
      DelayCommand(fDelay + 1.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFX2, oPC));
      DelayCommand(fDelay + 2.0, AssignCommand(oPC, ActionJumpToLocation(GetLocation(oSpawnPointBS))));
      DelayCommand(fDelay + 3.0, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectKnockdown(), oPC, 3.0));
      DelayCommand(fDelay + 4.0, ApplyEffectToObject (DURATION_TYPE_INSTANT, eFX3, oPC));
      DelayCommand(fDelay + 5.0, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectDazed(), oPC, 5.0));
      DelayCommand(fDelay + 6.0, ApplyEffectToObject (DURATION_TYPE_INSTANT, eFX4, oPC));
      DelayCommand(fDelay + 7.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectHeal(GetMaxHitPoints(oPC)), oPC));
    }
    // Checks to see if there has been no binding
    else
    {
      if(GetAlignmentGoodEvil(oPC) == ALIGNMENT_GOOD) sDestTagDF = "WP_GOOD_HOME";
      else if(GetAlignmentGoodEvil(oPC) == ALIGNMENT_EVIL) sDestTagDF = "WP_EVIL_HOME";
      else sDestTagDF = "WP_NEU_HOME";

      object oSpawnPointDF = GetObjectByTag(sDestTagDF);

      DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFX1, oPC));
      DelayCommand(fDelay + 1.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, eFX2, oPC));
      DelayCommand(fDelay + 2.0, AssignCommand(oPC, ActionJumpToLocation(GetLocation(oSpawnPointDF))));
      DelayCommand(fDelay + 3.0, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectKnockdown(), oPC, 3.0));
      DelayCommand(fDelay + 4.0, ApplyEffectToObject (DURATION_TYPE_INSTANT, eFX3, oPC));
      DelayCommand(fDelay + 5.0, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectDazed(), oPC, 5.0));
      DelayCommand(fDelay + 6.0, ApplyEffectToObject (DURATION_TYPE_INSTANT, eFX4, oPC));
      DelayCommand(fDelay + 7.0, ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectHeal(GetMaxHitPoints(oPC)), oPC));
    }
}
