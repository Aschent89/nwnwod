...
void main()
{
    ...
    //fTargetface = GetNormalizedDirection(fTargetface - 40.0);
    ...
    vector posDest = GetPosition(oTarget) - AngleToVector(fTargetface)*1.25;
    location lDest = Location(oArea, posDest, fPCFace);
    MoveToNewLocation ( lDest, oPC);

    // perform animation
    DelayCommand( 2.0, AssignCommand( oPC, ActionPlayAnimation(ANIMATION_LOOPING_MEDITATE, 1.0, 60000.0)));
