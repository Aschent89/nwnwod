...
************************* [Other - Behaviour/Generic] *************************/
    ...
    // HCR: This *MUST* be commented IN, in order for HCR NPC lootable corpses
    //      to work as expected.
...
************************* [User Defined and Shouts] ***************************/
 ...
 /*** Speakstrings - as it were, said under cirtain conditions % chance each time ***/
    ...
    // Morale
    AI_SetSpawnInSpeakArray(AI_TALK_ON_MORALE_BREAK, 0, 0, "No more!", "I'm outta here!", "Catch me if you can!");
        // Spoken at running point, if they run to a group of allies.
    AI_SetSpawnInSpeakArray(AI_TALK_ON_CANNOT_RUN, 0, 0, "Never give up! Never surrender!", "I've no where to run, so make my day!", "RRRAAAAA!!!");
    ...
    // Combat
    AI_SetSpawnInSpeakArray(AI_TALK_ON_COMBAT_ROUND_EQUAL, 0, 0, "Come on!", "You won't win!", "We are not equals! I am better!", "Nothing will stop me!");
    AI_SetSpawnInSpeakArray(AI_TALK_ON_COMBAT_ROUND_THEM_OVER_US, 0, 0, "I'll try! try! and try again!", "Tough man, are we?", "Trying out your 'skills'? Pathetic excuse!", "Nothing good will come from killing me!");
    AI_SetSpawnInSpeakArray(AI_TALK_ON_COMBAT_ROUND_US_OVER_THEM, 0, 0, "My strength is mighty then yours!", "You will definatly die!", "NO chance for you!", "No mercy! Not for YOU!");
        // Spoken each DetermineCombatRound. % is /1000. See readme for Equal/Over/Under values.
    AI_SetSpawnInSpeakArray(AI_TALK_ON_TAUNT, 0, 0, "You're going down!", "No need to think, let my blade do it for you!", "Time to meet your death!");
        // If the creature uses thier skill, taunt, on an enemy this will be said.

    // Event-driven.
    AI_SetSpawnInSpeakArray(AI_TALK_ON_PERCIEVE_ENEMY, 0, 0, "Stand and fight, lawbreaker!", "Don't run from the law!", "I have my orders!", "I am ready for violence!", "CHARGE!", "Time you died!");
        // This is said when they see/hear a new enemy, and start attacking them.
    AI_SetSpawnInSpeakArray(AI_TALK_ON_DAMAGED, 0, 0, "Ouch, damn you!", "Haha! Nothing will stop me!");
        // A random value is set to speak when damaged, and may fire same time as below ones.
    AI_SetSpawnInSpeakArray(AI_TALK_ON_PHISICALLY_ATTACKED, 0, 0, "Hah! Mear weapons won't defeat me!", "Pah! You cannot defeat me with such rubbish!");
        // This is said when an enemy attacks the creature with a melee/ranged weapon.
    AI_SetSpawnInSpeakArray(AI_TALK_ON_HOSTILE_SPELL_CAST_AT, 0, 0, "No one spell will stop me!", "Is that all you have!?!");
        // This is said when an enemy attacks the creature with a hostile spell.
    //I_SetSpawnInSpeakValue(AI_TALK_ON_DEATH, "Agggggg!");
