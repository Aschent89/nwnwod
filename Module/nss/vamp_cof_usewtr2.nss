...
void main(){
    ...
    if(!GetIsObjectValid(oItemToTake)){
        oItemToTake = GetItemPossessedBy(GetPCSpeaker(), "NW_IT_MSMLMISC15");
    }
    if(!GetIsObjectValid(oItemToTake)){
    ...
    if(GetLevelByClass(CLASS_TYPE_CLERIC, GetPCSpeaker()) + GetLevelByClass(CLASS_TYPE_DRUID, GetPCSpeaker()) > 0){
        iRoll += 2;
    }
