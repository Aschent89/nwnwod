// hcr3.1
// added not doing hunger checks for dying pcs.
// sr5.3
// Added code for Rangers and Druids to detect foul water.
//hc_inc_htf
//
...
void InitPCHTFLevels(object oPC)
{
    ...
    if (iHUNGERSYSTEM) {
        InitPCHTFLevelVarByType("HUNGERLEVEL"  + GetName(oPC) + GetPCPlayerName(oPC), INITHUNGERLEVEL);
        InitPCHTFLevelVarByType("THIRSTLEVEL"  + GetName(oPC) + GetPCPlayerName(oPC), INITTHIRSTLEVEL);
    }
    if (iFATIGUESYSTEM)
        InitPCHTFLevelVarByType("FATIGUELEVEL" + GetName(oPC) + GetPCPlayerName(oPC), INITFATIGUELEVEL);
}
...
int IsPCVeryHungryOrThirsty(object oPC)
{
    ...
    int HungerRating = IsPCHTFLevelByVarNameLow("HUNGERLEVEL" + GetName(oPC) + GetPCPlayerName(oPC),INITHUNGERLEVEL);
    int ThirstRating = IsPCHTFLevelByVarNameLow("THIRSTLEVEL" + GetName(oPC) + GetPCPlayerName(oPC),INITTHIRSTLEVEL);
...
{
    if (!GetIsPC(oPC)) return;
    SetLocalInt(GetModule(),"HUNGERLEVEL"  + GetName(oPC) + GetPCPlayerName(oPC), INITHUNGERLEVEL);
    SetLocalInt(GetModule(),"THIRSTLEVEL"  + GetName(oPC) + GetPCPlayerName(oPC), INITTHIRSTLEVEL);
    SetLocalInt(GetModule(),"FATIGUELEVEL" + GetName(oPC) + GetPCPlayerName(oPC), INITFATIGUELEVEL);
}
...
void ApplyHungerBonus(string sItemTag, object oPC)
{
    if (FindSubString(sItemTag,"FOOD") == -1)
        return;
    int oldlevel = GetLocalInt(GetModule(), "HUNGERLEVEL" + GetName(oPC) + GetPCPlayerName(oPC));
    ...
    SetLocalInt(GetModule(),"HUNGERLEVEL"  + GetName(oPC) + GetPCPlayerName(oPC), newlevel);
}
...
void ApplyThirstBonus(string sItemTag, object oPC)
{
    ...
    int oldlevel = GetLocalInt(GetModule(), "THIRSTLEVEL" + GetName(oPC) + GetPCPlayerName(oPC));
    ...
    SetLocalInt(GetModule(),"THIRSTLEVEL"  + GetName(oPC) + GetPCPlayerName(oPC), newlevel);
}

void ApplyEnergyBonus(string sItemTag, object oPC)
{
    int oldlevel = GetLocalInt(GetModule(), "FATIGUELEVEL" + GetName(oPC) + GetPCPlayerName(oPC));
    ...
    if (newlevel > oldlevel)
        SetLocalInt(GetModule(),"FATIGUELEVEL"  + GetName(oPC) + GetPCPlayerName(oPC), newlevel);
}
...
int UseFoodOrDrinkItem(object oPC, object oItem, string sItemTag = "")
{
    ...
    if (!iPOISON && !iDISEASE) {
        int hlevel = GetLocalInt(GetModule(), "HUNGERLEVEL" + GetName(oPC) + GetPCPlayerName(oPC));
        int tlevel = GetLocalInt(GetModule(), "THIRSTLEVEL" + GetName(oPC) + GetPCPlayerName(oPC));
        if (isDrink && isFood) {
...
void DoDeathByStarvation(object oPC)
{
    ...
    DeleteLocalInt(GetModule(),"STARVATIONSAVES" + GetName(oPC) + GetPCPlayerName(oPC));
    ResetHTFLevels(oPC);
...
void DoHungerChkOnPC(object oPC)
{
    ...
    if (newlevel == 1) {
        ...
        int iStarvationSaves = GetLocalInt(GetModule(),"STARVATIONSAVES" + GetName(oPC) + GetPCPlayerName(oPC));
        ...
        if (iFortSave) {
            ...
            SetLocalInt(GetModule(),"STARVATIONSAVES" + GetName(oPC) + GetPCPlayerName(oPC), iStarvationSaves + 5);
            if (AUTOEATDRINK)
                DoAutoEatDrink(oPC,"Food");
        }
        else
...
void DoDeathByDehydration(object oPC)
{
    ...
    effect eKillPC = EffectDamage(iHP + 11);
    ApplyEffectToObject(DURATION_TYPE_INSTANT,eKillPC,oPC);
    DeleteLocalInt(GetModule(),"DEHYDRATIONSAVES" + GetName(oPC) + GetPCPlayerName(oPC));
    ResetHTFLevels(oPC);
...
void DoThirstChkOnPC(object oPC)
{
    ...
    if (newlevel == 1) {
        ...
        int iDehydrationSaves = GetLocalInt(GetModule(),"DEHYDRATIONSAVES" + GetName(oPC) + GetPCPlayerName(oPC));
        ...
        if (iFortSave) {
            ...
            SetLocalInt(GetModule(),"DEHYDRATIONSAVES" + GetName(oPC) + GetPCPlayerName(oPC), iDehydrationSaves + 5);
            if (AUTOEATDRINK)
                DoAutoEatDrink(oPC,"Drink");
        }
        else
...
void DoFatigueChkOnPC(object oPC)
{
    ...
    if (newlevel == 1) {
        ...
        int iCollapseSaves = GetLocalInt(GetModule(),"FATIGUECOLLAPSESAVES" + GetName(oPC) + GetPCPlayerName(oPC));
        ...
        if (iFortSave) {
           ...
           SetLocalInt(GetModule(),"FATIGUECOLLAPSESAVES" + GetName(oPC) + GetPCPlayerName(oPC), iCollapseSaves + 5);
           AssignCommand (oPC,ActionPlayAnimation(ANIMATION_LOOPING_PAUSE_TIRED));
           AssignCommand (oPC,PlayVoiceChat(VOICE_CHAT_REST,oPC));
        }
        else {
           MakePlayerCollapse(oPC,COLLAPSEFROMEXHAUSTION);
           SetLocalInt(GetModule(),"FATIGUELEVEL" + GetName(oPC) + GetPCPlayerName(oPC), FloatToInt(INITFATIGUELEVEL*0.25));
        }
    }
}
...
void LoopHTFSystemChk()
{
    ...
    while (oPC != OBJECT_INVALID) {
        if (!GetIsDM(oPC) && GPS(oPC) == PWS_PLAYER_STATE_ALIVE) {
            if (iHUNGERSYSTEM) {
                DoHungerChkOnPC(oPC);
                DoThirstChkOnPC(oPC);
            }
