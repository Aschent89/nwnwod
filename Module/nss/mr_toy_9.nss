...
void main()
{
    ...
    switch (GetGender(OBJECT_SELF)) {
        case GENDER_FEMALE : nAppearance = BDSM_NUMBER_FOOT_CUFFS_F; break;
        case GENDER_MALE : nAppearance = BDSM_NUMBER_FOOT_CUFFS_M; break; }

    //Removal messages
    string sSpeak1 = sOwnerName + " removes a leather cuff from each ankle of " + sTargetName;
    string sOwnerMessage1 = "You remove the leather cuffs from the ankle of " + sTargetName;
    string sTargetMessage1 = sOwnerName + " removes the leather cuffs from your ankles.";

    //Add messages
    string sSpeak2 = sOwnerName + " fastens a leather cuff around each ankle of " + sTargetName;
    string sOwnerMessage2 = "You fasten a leather cuff around each ankle of " + sTargetName;
    string sTargetMessage2 = sOwnerName + " fastens a leather cuff around each of your ankles.";
    ...
    if (nToyInUse) {
        ...
        oModified = CopyItemAndModify(oChest, ITEM_APPR_TYPE_ARMOR_MODEL, ITEM_APPR_ARMOR_MODEL_LSHIN, 0);
        oModified2 = CopyItemAndModify(oModified, ITEM_APPR_TYPE_ARMOR_MODEL, ITEM_APPR_ARMOR_MODEL_RSHIN, 0);
    ...
    if (!nToyInUse) {
        //Alter the clothing
        oModified = CopyItemAndModify(oChest, ITEM_APPR_TYPE_ARMOR_MODEL, ITEM_APPR_ARMOR_MODEL_RSHIN, nAppearance);
        oModified2 = CopyItemAndModify(oModified, ITEM_APPR_TYPE_ARMOR_MODEL, ITEM_APPR_ARMOR_MODEL_LSHIN, nAppearance);
        //Equip the new clothing
        DelayCommand(0.8, AssignCommand(OBJECT_SELF, ActionEquipItem(oModified2, INVENTORY_SLOT_CHEST)));
        //Destroy the old clothing
        DelayCommand(1.3, DestroyObject(oModified));
