...
void main()
{
    ...
    vector posDest = GetPosition(oTarget) + AngleToVector(fTargetface)*0.36;
    location lDest = Location(oArea, posDest, fPCFace);
    MoveToNewLocation ( lDest, oPC);

    // perform animation
    DelayCommand( 1.0, AssignCommand( oPC, ActionPlayAnimation(ANIMATION_LOOPING_MEDITATE, 1.0, 60000.0)));
