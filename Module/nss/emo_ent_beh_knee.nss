...
void main()
{
    ...
    vector posDest = GetPosition(oTarget) + AngleToVector(fTargetface)*0.35;
    location lDest = Location(oArea, posDest, fPCFace);
    MoveToNewLocation ( lDest, oPC);

    // perform animation
    DelayCommand( 1.0, AssignCommand( oPC, ActionPlayAnimation(ANIMATION_LOOPING_GET_LOW, 0.5, 60000.0)));
